...
class JointsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # if idx >= len(self.db) - 1:
        #     idx -= 1  # Avoid overflow for the last index

        # db_rec = copy.deepcopy(self.db[idx])
        # db_rec_t1 = copy.deepcopy(self.db[idx +1])
        # print("db_rec===>" , db_rec)
        if idx >= len(self.db) - 1:
            # print(idx)
        # If last frame, duplicate the last frame
            db_rec = copy.deepcopy(self.db[idx])
            db_rec_t1 = copy.deepcopy(self.db[idx])
            (f"Warning: Using the last frame {idx} twice as no next frame exists.")
        else:
            # print("idx_else" , idx)
            db_rec = copy.deepcopy(self.db[idx])
            # print("dec_rec===>" , db_rec)
            db_rec_t1 = copy.deepcopy(self.db[idx + 1])
            # print("dec_rec_t1===>" , db_rec_t1)

        image_file = db_rec['image']
        # print("image_file===>" , len(image_file))
        image_file_t1 = db_rec_t1['image']

        if self.data_format == 'zip':
            from utils import zipreader
            data_numpy = zipreader.imread(
                image_file, cv2.IMREAD_COLOR | cv2.IMREAD_IGNORE_ORIENTATION)
            data_numpy_t1 = zipreader.imread(
                image_file_t1, cv2.IMREAD_COLOR | cv2.IMREAD_IGNORE_ORIENTATION) 
        else:
            data_numpy = cv2.imread(
                image_file, cv2.IMREAD_COLOR | cv2.IMREAD_IGNORE_ORIENTATION)
            data_numpy_t1 = cv2.imread(
                image_file_t1, cv2.IMREAD_COLOR | cv2.IMREAD_IGNORE_ORIENTATION)
        if 'source' in db_rec and db_rec['source'] == 'h36m':
            # crop image from 1002 x 1000 to 1000 x 1000 for h36m
            data_numpy = data_numpy[:1000]
            data_numpy_t1= data_numpy_t1[:1000]

        # if data_numpy or data_numpy_t1 is None:
        #     print('data_numpy is None')
        #     return None, None, None, None, None, None
        if data_numpy is None or data_numpy_t1 is None:
            print(f"data_numpy is None for index {idx} or {idx + 1}")
            return None, None, None, None, None, None

        if self.color_rgb:
            data_numpy = cv2.cvtColor(data_numpy, cv2.COLOR_BGR2RGB)
            data_numpy_t1 = cv2.cvtColor(data_numpy_t1, cv2.COLOR_BGR2RGB)
        ...
        joints_t1= db_rec_t1['joints_2d']
        joints_3d_t1 = db_rec_t1['joints_3d']
        joints_vis_t1 = db_rec_t1['joints_2d_vis']
        joints_3d_vis_t1 = db_rec_t1['joints_3d_vis']
        joints_3d_voxelpose_pred_t1 = db_rec_t1.get('joints_3d_voxelpose_pred')

        nposes = len(joints)
        assert nposes <= self.maximum_person, 'too many persons'

        nposes_t1 = len(joints_t1)
        assert nposes_t1 <= self.maximum_person, 'too many persons'

        height, width, _ = data_numpy.shape
        height_t1, width_t1, _ = data_numpy_t1.shape
        c = np.array([width / 2.0, height / 2.0])
        c_t1 = np.array([width_t1 / 2.0, height_t1 / 2.0])
        # print("c===>" , c , type(c ), c.shape)
        s = get_scale((width, height), self.image_size)
        s_t1 = get_scale((width_t1, height_t1), self.image_size)
        # print("s===>" , s , type(s) , c.shape)
        ...
        trans_t1 = get_affine_transform(c_t1, s_t1, r, self.image_size, inv=0)
        ...
        input_t1 = cv2.warpAffine(
            data_numpy_t1,
            trans_t1, (int(self.image_size[0]), int(self.image_size[1])),
            flags=cv2.INTER_LINEAR)

        if self.transform:
            inputs = self.transform(inputs)
            input_t1 = self.transform(input_t1)
        ...
        # for n1 in range(nposes_t1):
        #     for i1 in range(len(joints_t1[0])):
        #         if joints_vis_t1[n1][i1, 0] > 0.0:
        #             joints_t1[n1][i1, 0:2] = affine_transform(
        #                 joints_t1[n1][i1, 0:2], trans_t1)
        #             if (np.min(joints[n1][i1, :2]) < 0 or
        #                     joints_t1[n1][i1, 0] >= self.image_size[0] or
        #                     joints_t1[n1][i1, 1] >= self.image_size[1]):
        #                 joints_vis_t1[n1][i1, :] = 0
        for n1 in range(nposes_t1):
            for i1 in range(len(joints_t1[0])):
                if joints_vis_t1[n1][i1, 0] > 0.0:
                    joints_t1[n1][i1, 0:2] = affine_transform(joints_t1[n1][i1, 0:2], trans_t1)
                    if (np.min(joints_t1[n1][i1, :2]) < 0 or   # use joints_t1 here
                            joints_t1[n1][i1, 0] >= self.image_size[0] or
                            joints_t1[n1][i1, 1] >= self.image_size[1]):
                        joints_vis_t1[n1][i1, :] = 0
        ...
        aff_trans_t1 = np.eye(3, 3)
        aff_trans_t1[0:2] = trans_t1

        inv_aff_trans = np.eye(3, 3)
        inv_trans = get_affine_transform(c, s, r, self.image_size, inv=1)
        inv_aff_trans[0:2] = inv_trans

        inv_aff_trans_t1 = np.eye(3, 3)
        inv_trans_t1 = get_affine_transform(c_t1, s_t1, r, self.image_size, inv=1)
        inv_aff_trans_t1[0:2] = inv_trans_t1
        ...
        aug_trans_t1 = np.eye(3, 3)
        aug_trans_t1[0:2] = trans_t1
        ...
        cam_t1 = db_rec_t1['camera']
        cam_intri_t1 = np.eye(3, 3)
        cam_intri_t1[0, 0] = float(cam_t1['fx'])
        cam_intri_t1[1, 1] = float(cam_t1['fy'])
        cam_intri_t1[0, 2] = float(cam_t1['cx'])
        cam_intri_t1[1, 2] = float(cam_t1['cy'])
        cam_R = cam_t1['R']
        cam_T = cam_t1['T']
        cam_standard_T = cam_t1['standard_T']
        ...
        meta_t1 = {
            ...
            'camera_T': cam_T,
            'camera_standard_T': cam_standard_T,
            'affine_trans': aff_trans_t1,
            'inv_affine_trans': inv_aff_trans_t1,
            'aug_trans': aug_trans_t1,
        }
        # print("Joint input===>" , type(meta))
        # print("Joint meta===>" , type(meta_t1))
        # print("joint_meta====>" , len(meta), len(meta_t1))
        return inputs, input_t1, meta , meta_t1
    ...
    def generate_input_heatmap(self, joints):
        ...
        return target
print("end of JointsDataset new ====>")