...
class Panoptic(JointsDataset):
    ...
    #             # print("flow_3===>" , flow)
    #         else:
    #             # If flow file doesn't exist, create zero flow
    #             flow = torch.zeros((2, i.shape[1], i.shape[2]))
    #             # print("flow_else===>" , flow)
            
    #         # Append images, metadata and flow
    #         inputs.append(i)
    ...
    #     return inputs, input_t1, meta, meta_t1, flows
    ...
    def __getitem__(self, idx):
        ...
        for k in range(self.num_views):
            ...
            if os.path.exists(flow_path):
                ...
                flow = frame_utils.read_gen(flow_path)
                flow = np.array(flow).astype(np.float32)
                # print("flow_1===>" , np.unique(flow))
                
                # u = (flow[..., 0] - 32768.0) / 64.0
                # v = (flow[..., 1] - 32768.0) / 64.0
        ...
        # Stack flows and valids into single tensors
        # flows = torch.stack(flows, dim=0)  # Shape: [num_views, 2, H, W]
        # valids = torch.stack(valids, dim=0)  # Shape: [num_views, H, W]
