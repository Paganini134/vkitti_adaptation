...
# Get the project root (parent of RAFT3D directory)
project_root = os.path.abspath(os.path.join(script_dir, '../../'))
if project_root not in sys.path:
    sys.path.insert(0, project_root)

# Also add RAFT3D directory to path for relative imports
raft3d_dir = os.path.abspath(os.path.join(script_dir, ".."))
if raft3d_dir not in sys.path:
    sys.path.insert(0, raft3d_dir)
...
# Add root to system path so you can access lib.models (already added above)
...
def test_sceneflow(model):
    ...
    # print(f"DEBUG===> Loading dataset '{config.DATASET.TEST_DATASET}'")
    # print(f"DEBUG===> Dataset root: {config.DATASET.ROOT}")
    # print(f"DEBUG===> Test subset: {config.DATASET.TEST_SUBSET}")
    # print("test_dataset===>", 'dataset.' + config.DATASET.TEST_DATASET)
    ...
    total_frames = len(test_dataset)          # total “timesteps”
    start_idx    = int(0.7 * total_frames)    # 70% point ⇒ last 30%
    indices      = list(range(start_idx, total_frames))

    test_dataset_subset = Subset(test_dataset, indices)

    sampler_val = torch.utils.data.SequentialSampler(test_dataset_subset)
    ...
    # Unpack all items from the dataloader
        ...
        # Move all tensors in meta dictionaries to CUDA
        # This is critical to avoid device mismatch errors
        meta = [{k: v.cuda() if isinstance(v, torch.Tensor) else v
                 for k, v in t.items()} for t in meta]
        meta_t1 = [{k: v.cuda() if isinstance(v, torch.Tensor) else v
        ...
        intrinsics_t0 = torch.stack(intrinsics_t0_list, dim=0).cuda()  # [num_views, 3, 3]
        intrinsics_t1 = torch.stack(intrinsics_t1_list, dim=0).cuda()  # [num_views, 3, 3]
        ...
        intrinsics_both = torch.stack([intrinsics_t0, intrinsics_t1], dim=1)  # [num_views, 2, 3, 3]
        
        print(f"Debug - intrinsics_both shape: {intrinsics_both.shape}")
        
        # Add batch dimension: [num_views, batch_size, num_timesteps, 3, 3]
        intrinsics = intrinsics_both.unsqueeze(1)  # [num_views, 1, 2, 3, 3]
        
        index = torch.tensor(indices).cuda()
        
        print(f"Debug - Final intrinsics shape: {intrinsics.shape}")  # Should be [num_views, 1, 2, 3, 3]
        ...
        # Check if this is the multi-view format (num_views views) or 2*num_views views (num_views views x 2 timesteps)
        if image1.shape[0] == num_views:  # Single timestep per view
            # Use the images directly without reshaping
            image1_t0_list = [image1[i:i+1] for i in range(num_views)]  # List of num_views tensors [1,3,H,W]
            image2_t1_list = [image2[i:i+1] for i in range(num_views)]  # List of num_views tensors [1,3,H,W]
        elif image1.shape[0] == 2 * num_views:  # Two timesteps per view
            ...
            image1_t0_list = [image1_reshaped[:, i, 0] for i in range(num_views)]  # num_views tensors of [batch,3,H,W]
            image2_t1_list = [image2_reshaped[:, i, 1] for i in range(num_views)]  # num_views tensors of [batch,3,H,W]
        else:
            raise ValueError(f"Unexpected image1 shape: {image1.shape} for num_views={num_views}")
        ...
        # intrinsics_float32 shape is [num_views, 1, 2, 3, 3], we need [num_views, 3, 3]
        ...
        # We need depth for just the first timestep and num_views views: [num_views, 1, H, W]
        print(f"Debug - depth1 shape before flow computation: {depth1.shape}")
        print(f"Debug - Ts shape: {Ts.shape}")
        
        # If depth1 has 2*num_views views (num_views views × 2 timesteps), take only first num_views views
        if depth1.shape[0] == 2 * num_views:
            depth1_for_flow = depth1[:num_views]  # Take first num_views views (timestep 0)
            print(f"Debug - Using first {num_views} views, depth1_for_flow shape: {depth1_for_flow.shape}")
        else:
        ...
        if Ts.shape[1] != target_h or Ts.shape[2] != target_w:
            ...
            Ts_data = Ts.data  # [num_views, 512, 512, 7]
            Ts_data = Ts_data.permute(0, 3, 1, 2)  # [num_views, 7, 512, 512]
        ...
        # flow2d is [num_views, 2, 2, 512, 960], we need [num_views, 512, 960, 2]
        # Take the first element from the extra dimensions and transpose
        if flow2d.dim() == 5 and flow2d.shape[1] == 2 and flow2d.shape[2] == 2:
            flow2d = flow2d[:, 0, :, :, :].permute(0, 2, 3, 1)  # [num_views, 2, 512, 960] -> [num_views, 512, 960, 2]
        
        # flow3d is [num_views, 2, 3, 512, 960], we need [num_views, 512, 960, 3]
        if flow3d.dim() == 5 and flow3d.shape[1] == 2:
            flow3d = flow3d[:, 0, :, :, :].permute(0, 2, 3, 1)  # [num_views, 3, 512, 960] -> [num_views, 512, 960, 3]
        ...
        if len(flow2d.shape) == 5:  # Multi-view format [num_views, 1, 2, H, W]
            # Reshape to [num_views, 2, H, W] for interpolation
            flow2d = flow2d.squeeze(1)  # Remove singleton dimension -> [num_views, 2, H, W]
        ...
        if len(flow3d.shape) == 5:  # Multi-view format [num_views, 1, 3, H, W]
            # Reshape to [num_views, 3, H, W] for interpolation
            flow3d = flow3d.squeeze(1)  # Remove singleton dimension -> [num_views, 3, H, W]
        ...
        depth_final = torch.sum(flow3d_est**2, dim=-1).sqrt()  # [num_views, H, W] - use 3D flow magnitude as depth proxy
    ...
    for key in metrics_all:
        value = metrics_all[key] / count_all
    ...
    # Return aggregated validation metrics (including epe3d)
    return results_all
...
if __name__ == '__main__':
    ...
    # Load config file for MVGFormer
